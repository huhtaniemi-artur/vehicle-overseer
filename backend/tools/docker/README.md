...
- Then run setup in the container:
  ...
  - If neither is present, fallback:
    - `python3 -c "import urllib.request; print(urllib.request.urlopen('http://host.docker.internal:3100/api/srvcsetup?vin=CAR_001&token=PASTE_TOKEN').read().decode('utf-8'))" | bash`
